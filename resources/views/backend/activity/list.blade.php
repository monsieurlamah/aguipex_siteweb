            ...
            <div class="row">
                <div class="col">
                    <div class="btn-group" role="group"><a class="btn btn-success" role="button"
                            href="{{ route('activity.create') }}"><i
                                class="fas fa-plus-circle"></i>&nbsp;<strong>&nbsp;</strong>Ajouter une activité</a></div>
                                    ...
                                    <tbody>
                                        @forelse ($activities as $activity)
                                            <tr style="text-align:center">
                                                <td>{{ $activity->id }}</td>
                                                @if ($activity->image)
                                                <td><img src="{{asset('assets/activity/'.$activity->image)}}" height="50"
                                                ...
                                                <td>{{ $activity->type }}</td>
                                                <td>{{ $activity->date_activitie }}</td>
                                                <td><span
                                                        class="badge badge-pill badge-success">{{ $activity->status }}</span>
                                                </td>
                                                {{-- <td>
                                                    <a href="{{route('activity.edit', $activity->id)}}"><i class="fa fa-edit"></i></a>

                                                </td>
                                                <td><a href="{{route('activity.delete', $activity->id)}}"><i class="fa fa-trash"></i></a></td> --}}
                                                <td align="center">
                                                    <a href="{{route('activity.detail', $activity->id)}}"><button type="button"
                                        ...
                                        @empty
                                            <td class="text-center" colspan="9">Aucune activité trouvée</td>
                                        @endforelse
                                    </tbody>
                                </table>
                            </div>
