        ...
        <div class="row">
            <div class="col">
                <div class="btn-group" role="group"><a class="btn btn-success" role="button"
                        href="{{ route('equipe.create') }}"><i
                            class="fas fa-plus-circle"></i>&nbsp;<strong>&nbsp;</strong>Ajouter un employer</a></div>
            ...
            <div class="col col-md-12">
                <div class="card">
                    <div class="card-header">
                        <h4 class="card-title">Toutes l'équipe</h4>
                    </div>
                    <div class="card-body">
                                ...
                                <tbody>
                                    @forelse ($equipes as $equipe)
                                        <tr style="text-align:center">
                                            <td>{{ $equipe->id }}</td>
                                            @if ($equipe->image)
                                                <td><img src="{{asset('assets/equipe/'.$equipe->image)}}" height="50" width="50"
                                                        style="object-fit: cover" alt="{{ $equipe->title }}"></td>
                                            @endif
                                            <td>{{ $equipe->departement->name }}</td>
                                            <td>{{ $equipe->name }}</td>
                                            <td>{{ $equipe->email }}</td>
                                            <td>{{ $equipe->contact }}</td>
                                            <td>{{ $equipe->fonction }}</td>
                                            <td><span class="badge badge-pill badge-success">{{ $equipe->status }}</span>
                                            </td>
                                            <td>
                                                <a href="{{ route('equipe.edit', $equipe->id) }}"><i
                                                        class="fa fa-edit"></i></a>
                                            </td>
                                            <td><a href="{{ route('equipe.delete', $equipe->id) }}"><i
                                                        class="fa fa-trash"></i></a></td>
                                            <td align="center">
                                                <a href="{{ route('equipe.detail', $equipe->id) }}"><button
                                    ...
                                    @empty
                                        <td class="text-center" colspan="10">Aucun membre trouvé</td>
                                    @endforelse
                                </tbody>
                            </table>
                        </div>
