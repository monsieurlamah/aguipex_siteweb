        ...
        <div class="row">
            <div class="col">
                <div class="btn-group" role="group"><a class="btn btn-success" role="button"
                        href="{{ route('produit.create') }}"><i
                            class="fas fa-plus-circle"></i>&nbsp;<strong>&nbsp;</strong>Ajouter un produit</a></div>
                                ...
                                <tbody>
                                    @forelse ($produits as $produit)
                                        <tr style="text-align:center">
                                            <td>{{ $produit->id }}</td>
                                            @if ($produit->image)
                                                <td><img src="{{asset('assets/produit/'.$produit->image)}}" height="50" width="50"
                                                        style="object-fit: cover" alt="{{ $produit->title }}"></td>
                                            @endif
                                            <td>{{ $produit->categorie->name }}</td>
                                            <td>{{ $produit->title }}</td>
                                            <td><span class="badge badge-pill badge-success">{{ $produit->status }}</span>
                                            </td>
                                            <td>
                                                <a href="{{ route('product.edit', $produit->id) }}"><i
                                                        class="fa fa-edit"></i></a>
                                            </td>
                                            <td><a href="{{ route('product.delete', $produit->id) }}"><i
                                                        class="fa fa-trash"></i></a></td>
                                            <td align="center">
                                                <a href="{{ route('product.detail', $produit->id) }}"><button
                                    ...
                                    @empty
                                        <td class="text-center" colspan="8">Aucun produit trouvé</td>
                                    @endforelse
                                </tbody>
                            </table>
                        </div>
